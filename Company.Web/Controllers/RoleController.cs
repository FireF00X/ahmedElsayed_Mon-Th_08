        ...
        public async Task<IActionResult> Update(string id, RoleViewModel roleModel)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    role.Name = roleModel.Name.ToUpper();
            ...
            if(ModelState.IsValid)
            {
                foreach(var user in users)
                {
                    ...
                    if (appUser is not null) {
                        if (user.IsSelected && !await _userManager.IsInRoleAsync(appUser, roleId))
                            await _userManager.AddToRoleAsync(appUser, role.Name);
                        else if(!user.IsSelected && await _userManager.IsInRoleAsync(appUser,roleId))
